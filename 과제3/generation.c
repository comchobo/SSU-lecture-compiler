...
void gen_expression(A_NODE* node)
{
	...
	case N_EXP_IDENT:
		...
		case ID_PARM:
			...
			case T_POINTER:
				...
				break;
			case T_ARRAY:
				if(id->kind == ID_VAR) gen_code_i(LDA, id->level, id->address);
				else gen_code_i(LOD, id->level, id->address);
				break;
			case T_STRUCT:
			case T_UNION:
	...
	case N_EXP_POST_INC:
		...
		else gen_code_i(LDX, 0, 0);
		if (!isPointerOrArrayType(node->type)) {
			gen_code_i(LITI, 0, node->type->element_type->size);
			gen_code_i(ADDI, 0, 0);
		}
		else if(isFloatType(node->type)) gen_code_i(INCF, 0, 0);
		else gen_code_i(INCI, 0, 0);
		if (node->type->size == 1) gen_code_i(STOB, 0, 0);
		else gen_code_i(STO, 0, 0);
		break;
	case N_EXP_POST_DEC:
		gen_expression(node->clink);
		gen_expression_left(node->clink);
		...
		else if (isFloatType(node->type)) gen_code_i(DECF, 0, 0);
		else gen_code_i(DECI, 0, 0);
		if (node->type->size == 1) gen_code_i(STOB, 0, 0);
		else gen_code_i(STO, 0, 0);
		break;
	...
	case N_EXP_PRE_INC:
		...
		if (node->type->size == 1) gen_code_i(STXB, 0, 0);
		else gen_code_i(STX, 0, 0);
		break;
	...
	case N_EXP_CAST:
		gen_expression(node->rlink);
		if (node->type != node->rlink->type) if (isFloatType(node->type))
			gen_code_i(CVTF, 0, 0);
		else if (isFloatType(node->rlink->type)) gen_code_i(CVTI, 0, 0);
		break;
	case N_EXP_MUL:
		gen_expression(node->llink);
		gen_expression(node->rlink);
		if (isFloatType(node->type)) gen_code_i(MULF, 0, 0);
		else gen_code_i(MULI, 0, 0);
		break;
	case N_EXP_DIV:
		gen_expression(node->llink);
		gen_expression(node->rlink);
	...
	case N_EXP_SUB:
		...
		else gen_code_i(SUBI, 0, 0);

		break;
	case N_EXP_LSS:
		gen_expression(node->llink);
		gen_expression(node->rlink);
		if (isFloatType(node->llink->type)) gen_code_i(LSSF, 0, 0);
		else gen_code_i(LSSI, 0, 0);
		break;
	case N_EXP_GTR:
		gen_expression(node->llink);
		gen_expression(node->rlink);
	...
	case N_EXP_ASSIGN:
		...
		else gen_code_i(STX, 0, i % 4 ? i / 4 + 1 : i / 4);
		break;
	default:
		gen_error(100, node->line);
		break;
...
void gen_expression_left(A_NODE* node)
{
	...
	case N_EXP_IDENT:
		...
		case ID_PARM:
			...
			default:gen_error(13, node->line);
				break;
			}
			break;
		case ID_FUNC:
