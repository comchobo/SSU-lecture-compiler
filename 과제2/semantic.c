...
int sem_statement(A_NODE *, int, A_TYPE *);
int sem_statement_list(A_NODE *, int, A_TYPE *); //임의로 수정
...
int literal_no=0;
int literal_size=0;
BOOLEAN Global_SW = TRUE;
BOOLEAN Global_BRK = TRUE;
BOOLEAN Global_CNT = TRUE;//임의로 수정
...
A_TYPE* sem_expression(A_NODE* node)
{
	...
	case N_EXP_ARRAY:
		...
		t1 = node->llink->type;
		t2 = node->rlink->type;
		if (isPointerOrArrayType(t1)) result = t1->element_type;
		else semantic_error(32, node->line);
		// 배열참조식의 좌우측 타입을 검사
		// 수식의 lvalue 여부 판단
		if (!isIntegralType(t2)) semantic_error(29, node->line);
		if (!isArrayType(result)) lvalue = TRUE;
		break;
	case N_EXP_STRUCT:
		...
		else semantic_error(37, node->line);
		node->rlink = id;
		break;
	case N_EXP_ARROW:
		t = sem_expression(node->llink);
	...
	case N_EXP_FUNCTION_CALL:
		...
		else semantic_error(21, node->line);
		break;
	case N_EXP_POST_INC:
	case N_EXP_POST_DEC:
		...
		if (!isModifiableLvalue(node->clink)) semantic_error(60, node->line);
	...
	case N_EXP_CAST:
		...
		if (isAllowableCastingConversion(result, t))  semantic_error(58, node->line);
		break;
	case N_EXP_SIZE_TYPE:
		t = node->clink;
		i = sem_A_TYPE(t);
		// check if incomplete array, function, void
		if (isArrayType(t) && t->size == 0 || isFunctionType(t) || isVoidType(t)) semantic_error(39, node->line);
		else
			node->clink = i;
	...
	case N_EXP_SIZE_EXP:
		...
		if ((node->clink->name != N_EXP_IDENT || ((A_ID*)node->clink->clink)->kind != ID_PARM) &&
			(isArrayType(t) && t->size == 0 || isFunctionType(t))) semantic_error(39, node->line);
		else
			node->clink = t->size;
		result = int_type; break;
	case N_EXP_PLUS:
	case N_EXP_MINUS:
		...
		else semantic_error(13, node->line);
		break;
	case N_EXP_NOT:
		t = sem_expression(node->clink);
		// 수식의 타입 검사, 변환
		...
		else semantic_error(27, node->line);
		break;
	case N_EXP_AMP:
		t = sem_expression(node->clink);
		// 수식의 타입 검사, 결정
		...
		else semantic_error(60, node->line);
		break;
	case N_EXP_STAR:
		t = sem_expression(node->clink);
		node->clink = convertUsualUnaryConversion(node->clink);
		...
		else semantic_error(31, node->line);
		break;
	case N_EXP_PRE_INC: case N_EXP_PRE_DEC:
		...
		if (isScalarType(result)) semantic_error(27, node->line);
		if (!isModifiableLvalue(node->clink)) semantic_error(60, node->line);
	break; case N_EXP_MUL: case N_EXP_DIV:
		t1 = sem_expression(node->llink);
		t2 = sem_expression(node->rlink);
		// 좌우 수식의 타입 검사와 변환
		...
		result = int_type;
	break; case N_EXP_MOD:
		t1 = sem_expression(node->llink);
		t2 = sem_expression(node->rlink);
		// 좌우 수식의 타입 검사와 변환
		if (isIntegralType(t1) && isIntegralType(t2)) result = convertUsualBinaryConversion(node);
		else semantic_error(29, node->line);
	result = int_type; break; case N_EXP_ADD:
		t1 = sem_expression(node->llink);
		t2 = sem_expression(node->rlink);
		// 좌우 수식의 타입 검사와 변환
		if (isArithmeticType(t1) && isArithmeticType(t2)) result = convertUsualBinaryConversion(node);
		else if (isPointerType(t1) && isIntegralType(t2)) result = t1;
		else if (isIntegralType(t1) && isPointerType(t2)) result = t2;
		else semantic_error(24, node->line);
	break; case N_EXP_SUB:
		t1 = sem_expression(node->llink);
		t2 = sem_expression(node->rlink);
		// 좌우 수식의 타입 검사와 변환
		if (isArithmeticType(t1) && isArithmeticType(t2)) result = convertUsualBinaryConversion(node);
	...
	case N_EXP_LSS: case N_EXP_GTR: case N_EXP_LEQ: case N_EXP_GEQ:
		...
		t2 = sem_expression(node->rlink);
		// 좌우 수식의 타입 검사와 변환
		if (isArithmeticType(t1) && isArithmeticType(t2)) t = convertUsualBinaryConversion(node);
		else if (isPointerType(t1) && isIntegralType(t2)) semantic_error(40, node->line);
		result = int_type;
		break;
	case N_EXP_NEQ: case N_EXP_EQL:
		...
		result = int_type;
		break;
	case N_EXP_AND: case N_EXP_OR:
		t = sem_expression(node->llink);
		// 좌우 수식의 타입 검사와 변환
		if (isScalarType(t)) node->llink = convertUsualUnaryConversion(node->llink);
		else semantic_error(27, node->line);
		result = int_type;
		break;
	case N_EXP_ASSIGN:
		...
		else semantic_error(58, node->line);
		break;
	default:
		semantic_error(90, node->line);
		break;
...
void sem_arg_expr_list(A_NODE* node, A_ID* id)
{
	...
	switch (node->name) {
	case N_ARG_LIST:
		if (id == 0) semantic_error(34, node->line);
		else {
			if (id->type) {
				t = sem_expression(node->llink); 
				node->llink = convertUsualUnaryConversion(node->llink); 
	...
	break; 
	case N_ARG_LIST_NIL:
		if (id && id->type);// semantic_error(35,node->line);// check if '...' argument semantic_error(35,node->line);
		break; 
	default:
		printf("not matches\n");
		semantic_error(90, node->line);
...
int sem_statement(A_NODE* node, int addr, A_TYPE* ret)
{
	int local_size = 0, i;
	A_LITERAL lit;
	A_TYPE* t;
	...
	case N_STMT_LABEL_CASE:
		...
		local_size = sem_statement(node->rlink, addr, ret);
		break;
	case N_STMT_LABEL_DEFAULT:
		// 명령문 사용가능 여부 판단
		if (Global_SW == FALSE) semantic_error(71, node->line);
		local_size = sem_statement(node->clink, addr, ret);
		break;
	case N_STMT_COMPOUND:
		if (node->llink)
			local_size = sem_declaration_list(node->llink, addr);
		local_size += sem_statement_list(node->rlink, local_size + addr, ret);
		break;
	case N_STMT_EMPTY:
		break;
	...
	case N_STMT_IF:
		t = sem_expression(node->llink);
		// 수식의 타입 검사
		if (isScalarType(t)) node->llink = convertScalarToInteger(node->llink);
		else semantic_error(50, node->line);
		Global_SW = FALSE;
		local_size = sem_statement(node->rlink, addr, ret);
		break;
	case N_STMT_IF_ELSE:
		...
		Global_SW = FALSE;
		local_size = sem_statement(node->clink, addr, ret);
		i = sem_statement(node->rlink, addr, ret);
		// 명령문 내부 선언문의 크기 결정
	...
	case N_STMT_SWITCH:
		t = sem_expression(node->llink);
		// 수식의 타입 검사
		if (!isIntegralType(t)) semantic_error(50, node->line);
		Global_SW = TRUE;
		Global_BRK = TRUE;
		local_size = sem_statement(node->rlink, addr, ret);
		break;
	case N_STMT_WHILE:
		...
		break;
	case N_STMT_DO:
		local_size = sem_statement(node->llink, addr, ret);
		t = sem_expression(node->rlink);
		// 수식의 타입 검사
		if (isScalarType(t)) node->rlink = convertScalarToInteger(node->rlink);
		else semantic_error(50, node->line);
		break;
	case N_STMT_FOR:
	...
	case N_STMT_CONTINUE:
		// 명령문 사용가능 여부 판단
		if (Global_CNT == FALSE) semantic_error(74, node->line);
		break;
	case N_STMT_BREAK:
		// 명령문 사용가능 여부 판단
		if (Global_BRK == FALSE) semantic_error(73, node->line);
		break;
	case N_STMT_RETURN:
		if (node->clink) {
			t = sem_expression(node->clink);
...
void sem_for_expression(A_NODE* node)
		{
			...
			case N_FOR_EXP:
				...
				if (node->clink) {
					t = sem_expression(node->clink);
					// 수식의 타입 검사
					if (isScalarType(t)) node->clink = convertScalarToInteger(node->clink);
					else semantic_error(49, node->line);
...
int sem_statement_list(A_NODE* node, int addr, A_TYPE* ret)
{
	int size, i;
	switch (node->name) {
	case N_STMT_LIST:
		size = sem_statement(node->llink,addr,ret);
		i = sem_statement_list(node->rlink, addr, ret);
		if (size < i) size = i;
	break; case N_STMT_LIST_NIL:
		size = 0;
	break; default:
		semantic_error(90, node->line);
		break;
...
int sem_A_TYPE(A_TYPE* t)
{
	...
	case T_ENUM:
		...
		while (id) {
			...
			if (id->init) {
				...
				if (!isIntType(lit.type)) semantic_error(81, id->line);
				i = lit.value.i;
			}
			id->init = i++;
			id = id->link;
	...
	case T_FUNC:
		...
		i = sem_A_TYPE(tt);
		// check return type
		if (isArrayType(tt) || isFunctionType(tt)) semantic_error(85, t->line);
		i = sem_declaration_list(t->field, 12) + 12; 
		if (t->expr) { 
			Global_BRK = FALSE;
			Global_CNT = FALSE;
			Global_SW = FALSE;
			i = i + sem_statement(t->expr, i, t->element_type);
		}
		t->local_var_size = i;
		break; 
	case T_POINTER:
...
break; case ID_FIELD:
	i = sem_A_TYPE(id->type);
	// 필드의 타입 검사
	if (isFunctionType(id->type) || isVoidType(id->type)) semantic_error(84, id->line);
	if (i % 4) i = i / 4 * 4 + 4;
	id->address = addr;
	size = i;
break; case ID_FUNC:
...
break; case N_EXP_STRING_LITERAL : case N_EXP_ARRAY : case N_EXP_FUNCTION_CALL :
 case N_EXP_STRUCT :
 case N_EXP_ARROW :
 case N_EXP_POST_INC : case N_EXP_PRE_INC : case N_EXP_POST_DEC : case N_EXP_PRE_DEC : case N_EXP_AMP : case N_EXP_STAR : case N_EXP_NOT :
semantic_error(18,node->line);
break; case N_EXP_MINUS :
...
semantic_error(18,node->line);
break; case N_EXP_SIZE_EXP :
t=sem_expression(node->clink);
result.type=int_type;
result.value.i=t->size; break; case N_EXP_SIZE_TYPE :
result.type=int_type;
result.value.i=sem_A_TYPE(node->clink);
break; case N_EXP_CAST :
result=getTypeAndValueOfExpression(node->rlink);
result=checkTypeAndConvertLiteral(
result,(A_TYPE*)node->llink,node->line); break; case N_EXP_MUL :
...
semantic_error(18,node->line); break; case N_EXP_MOD :
result=getTypeAndValueOfExpression(node->llink);
r=getTypeAndValueOfExpression(node->rlink);
if (result.type==int_type && r.type==int_type)
result.value.i=result.value.i%r.value.i;
else
semantic_error(18,node->line); break; case N_EXP_ADD :
result=getTypeAndValueOfExpression(node->llink);
r=getTypeAndValueOfExpression(node->rlink);
if (result.type==int_type && r.type==int_type){
result.type=int_type;
...
semantic_error(18,node->line); break; case N_EXP_SUB :
result=getTypeAndValueOfExpression(node->llink);
r=getTypeAndValueOfExpression(node->rlink);
if (result.type==int_type && r.type==int_type){
result.type=int_type;
...
semantic_error(18,node->line); break; case N_EXP_LSS : case N_EXP_GTR : case N_EXP_LEQ : case N_EXP_GEQ : case N_EXP_NEQ : case N_EXP_EQL : case N_EXP_AND :
...
break; case 21:printf("illegal type in function call expression\n");
...
break;
// erros in statement case 49:printf("scalar type expr required in middle of for-expr\n");
